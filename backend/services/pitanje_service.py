...
VISOKOPRIORITETNE_KATEGORIJE = {"cistoca", "zivotni_stil"}
...
class PitanjeService:
    ...
    def create_pitanje(self, *, tekst_pitanja: str, kategorija: str,
        ...
        self._validate_kapacitet_kategorije(kategorija, tezina, postojeci_id=None)
        p = self.repo.create(
    ...
    def update_pitanje(self, pitanje_id: int, **changes) -> Pitanje:
        ...
        if (nova_kat, nova_tez) != (p.kategorija, p.tezina):
            self._validate_kapacitet_kategorije(nova_kat, nova_tez, postojeci_id=p.pitanje_id)
